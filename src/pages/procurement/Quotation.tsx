...
Use date : ${eventDate}`
  ...
  // Copy image from table row (without modal open) - uses the supplier entry saved for this quotation
  const handleCopyFromTable = async (quotation: MockQuotation) => {
    ...
    const savedEntries: FactoryEntry[] = (quotation as any).rawDetails?.supplierEntries || [];
    const winnerEntry = savedEntries.find(e => e.isWinner)
      || savedEntries.find(e => e.factoryValue === winnerFactory)
      || savedEntries[0];

    if (!winnerEntry) {
      toast.error("ไม่พบข้อมูลราคาผู้ผลิตที่บันทึกไว้สำหรับงานนี้");
      return;
    }

    await handleCopyRowAsImage(winnerEntry, 0, quotation);
  };
                    ...
                    {(tabValue === "approval" || tabValue === "completed") && (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const savedEntries: FactoryEntry[] = (quotation as any).rawDetails?.supplierEntries || [];
                          setSummarySupplierEntries(savedEntries);
