...
foreach ($path_parts as $key => $part) {
    if ($part === 'tasks2.php' && isset($path_parts[$key + 1]) && $path_parts[$key + 1] !== '') {
        $id = $path_parts[$key + 1];
    }
}
if (!$id && isset($_GET['id'])) {
...
if ($method === 'GET') {
    if ($id) {
        $stmt = $conn->prepare("SELECT * FROM tasks_test2 WHERE id = ?");
        ...
        $sub_stmt = $conn->prepare("SELECT id, name, is_completed FROM sub_tasks_test2 WHERE task_id = ? ORDER BY updated_at ASC");
    ...
    $sql = "SELECT * FROM tasks_test2 $where_sql ORDER BY sequence ASC, updated_at ASC";
    ...
    if (!empty($task_ids)) {
        ...
        $subResult = $conn->query("SELECT id, task_id, name, is_completed FROM sub_tasks_test2 WHERE task_id IN ($ids_str) ORDER BY updated_at ASC");
        if ($subResult) {
...
if ($method === 'POST') {
    ...
    if (isset($data['action']) && $data['action'] === 'reorder' && !empty($data['orders']) && is_array($data['orders'])) {
        ...
        try {
            $update_stmt = $conn->prepare("UPDATE tasks_test2 SET sequence = ? WHERE id = ?");
    ...
    $check = $conn->prepare("SELECT id FROM tasks_test2 WHERE id = ?");
    ...
    // Get max sequence
    $res_seq = $conn->query("SELECT MAX(sequence) as max_seq FROM tasks_test2");
    ...
    try {
        $stmt = $conn->prepare(
            "INSERT INTO tasks_test2 (id, name, type, implementation_status, uat_status, committee_status, cloud_status, e2e_status, note, sequence)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        ...
        if (!empty($data['subTasks']) && is_array($data['subTasks'])) {
            $sub_stmt = $conn->prepare("INSERT INTO sub_tasks_test2 (id, task_id, name, is_completed) VALUES (?, ?, ?, ?)");
    ...
    if (!empty($data)) {
        ...
        try {
            if (!empty($fields)) {
                $params[] = $id;
                $types .= 's';
                $stmt = $conn->prepare("UPDATE tasks_test2 SET " . implode(", ", $fields) . " WHERE id = ?");
                $stmt->bind_param($types, ...$params);
                $stmt->execute();
            }

            if (array_key_exists('subTasks', $data) && is_array($data['subTasks'])) {
                $del_stmt = $conn->prepare("DELETE FROM sub_tasks_test2 WHERE task_id = ?");
                $del_stmt->bind_param("s", $id);
                $del_stmt->execute();

                if (!empty($data['subTasks'])) {
                    $in_stmt = $conn->prepare("INSERT INTO sub_tasks_test2 (id, task_id, name, is_completed) VALUES (?, ?, ?, ?)");
                    foreach ($data['subTasks'] as $st) {
                        $st_id = !empty($st['id']) ? $st['id'] : uniqid('st_');
                        $st_name = $st['name'] ?? 'Untitled';
                        $st_comp = isset($st['percentCompleted']) ? (int) $st['percentCompleted'] : (!empty($st['isCompleted']) ? 100 : 0);
...
if ($method === 'DELETE') {
    ...
    $stmt = $conn->prepare("DELETE FROM tasks_test2 WHERE id = ?");
    $stmt->bind_param("s", $id);
