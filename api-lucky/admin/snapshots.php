...
if ($method === 'GET') {
    if ($id) {
        $stmt = $conn->prepare("SELECT * FROM snapshots_test WHERE id = ?");
    ...
    $sql = "SELECT id, snapshot_date, total_tasks, completed_tasks, overall_progress, created_at, snapshot_data FROM snapshots_test ORDER BY snapshot_date DESC, created_at DESC";
    $result = $conn->query($sql);

    $snapshots = [];
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $items = json_decode($row['snapshot_data'], true) ?: [];
            ...
            foreach ($items as $item) {
                ...
                if ($status === 'Done')
                    $doneCount++;
                else if ($status === 'Done เก็บ Defect')
                    $defectCount++;
                else if ($status === 'รอ UAT')
                    $uatCount++;
                else if ($status === 'Dev')
                    $devCount++;
            }
...
if ($method === 'POST') {
    ...
    // Use frontend-calculated values if provided (matches Dashboard1 logic)
    if (isset($data['overallProgress'])) {
        $total_tasks = isset($data['totalTasks']) ? (int) $data['totalTasks'] : count($data['items']);
        $completed_tasks = isset($data['completedTasks']) ? (int) $data['completedTasks'] : 0;
        $overall_progress = (int) $data['overallProgress'];
    } else {
        // Fallback: count tasks by implementationStatus (same as Dashboard1)
        $total_tasks = count($data['items']);
        $completed_tasks = 0;
        foreach ($data['items'] as $item) {
            if (
                isset($item['implementationStatus']) &&
                ($item['implementationStatus'] === 'Done' || $item['implementationStatus'] === 'Done เก็บ Defect')
            ) {
                $completed_tasks++;
            }
        }
    ...
    $stmt = $conn->prepare("INSERT INTO snapshots_test (id, snapshot_date, total_tasks, completed_tasks, overall_progress, snapshot_data) VALUES (?, ?, ?, ?, ?, ?)");
...
if ($method === 'DELETE') {
    ...
    $stmt = $conn->prepare("DELETE FROM snapshots_test WHERE id = ?");
    $stmt->bind_param("s", $id);
